...
author: "Ildi Czeller (@czeildi) and Jenő Pál (@paljenczy)"
date: "May 2018"
output:
  html_document:
    df_print: paged
...
Side effects mean that things happen silently: a plot gets saved, a variable's
value changes globally, etc. Avoid them or make them explicit.
```{r, class.source="bg-danger"}
...
You don't have to overdo refactoring (don't create a new function for `dplyr::filter`),
...
shortcuts <- rbind(
  ...
  data.frame("Description" = "Reindent lines",
             "Windows and Linux" = "Ctrl+I",
             "Mac" = "Command+I")
)

shortcuts %>%
  kable("html", col.names = c("Description", "Windows and Linux", "Mac")) %>%
